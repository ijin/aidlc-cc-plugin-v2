...
**Path convention.** All framework paths in this document (`skills/...`, `${CLAUDE_PLUGIN_ROOT}/aidlc-common/...`) are relative to the AI-DLC install root: `.kiro/` for Kiro, `.claude/` for Claude, etc. Paths under `aidlc-docs/...` live in the user's project, not the install root.

Inputs: human intent, `${CLAUDE_PLUGIN_ROOT}/skills/aidlc-orchestrator/CATALOGUE.md`, `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-folder-structure.md`, and `aidlc-docs/aidlc-state.md` if it exists.
...
When referring to sub-agents in prose, use the form `<stage>-builder` / `<stage>-validator`. Append the skill name only when one stage maps to multiple skills (e.g. `build-security-test-builder`). The actual Agent tool sub-agent call still targets `aidlc-builder-agent` / `aidlc-validator-agent` — the friendly name is display-only.

Workflow composition itself — the rules for selecting and ordering skills — lives in `${CLAUDE_PLUGIN_ROOT}/skills/aidlc-workflow-composition/SKILL.md`. Run that skill rather than reasoning about composition in this protocol.
...
Use the Agent tool to invoke the `aidlc-builder-agent` subagent. Include in the prompt:

- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/protocols/aidlc-builder-protocol.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/SKILL.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/validation-spec.md`
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-folder-structure.md`
...
Use the Agent tool to invoke the `aidlc-validator-agent` subagent. Include in the prompt:

- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/protocols/aidlc-validator-protocol.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/validation-spec.md`
- Artifact paths (from builder output or state)
- Answered question file path
- Skill output directory path
- Skill scripts directory path (`${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/scripts/`) if it exists
...
```
node ${CLAUDE_PLUGIN_ROOT}/aidlc-common/scripts/aidlc-process-checker.js --from-state <intent-dir>/state/process-checkpoint.json
```
...
A SubagentStop hook (bundled with this plugin) reminds you to run `process_checker` after every sub-agent call.
...
State file format, valid states, and transitions: `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-state-schema.md`.
...
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/protocols/aidlc-builder-protocol.md` — builder behaviour
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/protocols/aidlc-validator-protocol.md` — validator behaviour
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-state-schema.md` — state format, valid states, transitions, attempt counter
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-folder-structure.md` — directory layout
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-workflow-format.md` — `workflow.md` syntax
- `${CLAUDE_PLUGIN_ROOT}/aidlc-common/conventions/aidlc-question-format.md` — clarification question format
- `${CLAUDE_PLUGIN_ROOT}/skills/aidlc-orchestrator/CATALOGUE.md` — available skills and their flags
- `${CLAUDE_PLUGIN_ROOT}/skills/aidlc-workflow-composition/SKILL.md` — composition rules, presentation, and examples
